<?php
/**
	* Peter Bagi's Scraper Toolbox
	* Copyright: Peter Bagi 2013
 */
 ...
 * @param string $msg
 */
function error($msg) {header("HTTP/1.0 502 Not Found");exit($msg);}


function clearCache($maxAge = 1440) {
	$files = glob(BASE_DIR."/_cache/"."*");
...
/**
 * 
 * shows interim result in a fixed div overlay
 * @param string $stat
 */
function showStats($stat) {
	global $statContainerExists;
	if (!$statContainerExists) {
		echo "<div id='stats' style='position:fixed;top:5px; right:5px; background:white; border: 1px solid silver;padding:5px;'>$stat</div>";
		$statContainerExists = true;
	} else {
		echo "<script type='text/javascript'>document.getElementById('stats').innerHTML = '$stat';</script>";
	}
}
...
/**
 * 
 * get page load performance for a specified URL
 * returns an array with http_code, repsone_time & total_time
 * @param string $url
 */
function getPerformance($url) {
	...
	curl_setopt($ch, CURLOPT_USERAGENT, "MozillaPBA/1.0 (Performance Check Bot)");
	...
	$result['repsonse_time'] = str_replace(".",",",$info['starttransfer_time']);
	$result['total_time'] = str_replace(".",",",$info['total_time']);
	$result['repsonse_time_ms'] = $info['starttransfer_time']*1000;
	$result['total_time_ms'] = $info['total_time']*1000;
	return $result;
}
...
/**
 * returns an array of strings with each line of a file
 * usage example: foreach( lines("keywords.txt") as $line ) { }
 * @param string $filename
 * @param string $delimiter
 */
function lines($filename,$delimiter = "\n") {
...
/**
 * returns true if a substring is found in another string 
 * alias of contains() function
 * @param string $haystack
 * @param string $needle
 */
function str_contains($haystack,$needle, $ignorecase=true){
	return contains($needle, $haystack, $ignorecase=true);
}
...
/**
 * 
 * returns web page content and caches for 1 day or for the selected period
 * @param string $url
 * @param int $maxAge
 */
function getContentCached($url,$maxAge = 1440) { //maxAge in minutes -- default = 1 day
	if (!is_dir(BASE_DIR."/_cache/")) {mkdir(BASE_DIR."/_cache/");}
	$cachedFile = BASE_DIR."/_cache/".md5($url).".html";
//debug($cachedFile);
//die();
...
/**
 * returns web page content without caching
 * @param $url
 */
...
/**
 * returns html dom object of a standard cached $url
 * @param $url
 */
function getHTML($url,$inEncoding = "UTF-8") {
	$outEncoding = "UTF-8";
	return str_get_html(iconv($inEncoding,$outEncoding,getContentCached($url)));
}

/**
 * returns simplexml object of a standard cached $url
 * @param $url
 */
function getXML($url) {
	$content = getContentCached($url);
	$xml = simplexml_load_string(str_replace('xmlns=','ns=',$content));
	return $xml;
}

/**
 * returns plaintext value of a simplexml object
 * @param $url
 */
function getValue($var) {
	if (gettype($var) == "string") return $var;
	else if (gettype($var) == "array") return getValue($var[0]);
	else if (gettype($var) == "object") return getValue($var."");
}

/**
 * fetch given $url and return
 * @param $url
 * @return mixed
 */
...
/**
 * Log all requests to a file, sorted by date
 * @param $url
 */
function _log($url) {
	if (!is_dir(BASE_DIR."/_logs/")) {mkdir(BASE_DIR."/_logs/");}
	$f = fopen(BASE_DIR."/_logs/curl-".date("Y-m-d").".log",'a');
